...
class MultiDraftEval(Decoding):
    ...
    def eval(self):
        ...
        def launch_drafts_dynamic(tokenizer, request_queue, response_queues, total_drafts, max_concurrent=32):
            running_procs = []
            current_id = 0

            while current_id < total_drafts or running_procs:
                # 清理已结束的进程
                running_procs = [p for p in running_procs if p.is_alive()]

                # 启动新进程补满
                while current_id < total_drafts and len(running_procs) < max_concurrent:
                    proc = Process(
                    ...
                    proc.start()
                    print(f"Started draft {current_id}")
            ...
            print("All draft processes completed.")

        # 启动 target_model 中心验证者进程
        target_proc = Process(target=self.run_target_process_batching, args=(self.tokenizer, request_queue, response_queues))
        target_proc.start()
        ...
        #     proc = Process(target=self.run_draft_process, args=(self.tokenizer, request_queue, response_queues, i))
        #     proc.start()
        ...
        launch_drafts_dynamic(self.tokenizer, request_queue, response_queues, self.args.num_drafts, max_concurrent=32)
